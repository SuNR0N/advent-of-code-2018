...
function calculateTupleForID(id: string): [number, number] {
  const occurrences: Map<string, number> = new Map();
  for (const letter of id) {
    const value = occurrences.get(letter);
    occurrences.set(letter, value ? value + 1 : 1);
  }
...
const solution = calculateChecksum(ids);
printSolution(__filename, `The checksum of the list of box IDs: ${solution}`);