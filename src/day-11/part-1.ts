...
function calculateTotalPower(x: number, y: number, sn: number): number {
  const maxX = x + 3;
  const maxY = y + 3;
  let power = 0;
...
function calculatePowerLevel(x: number, y: number, sn: number): number {
  const rackId = x + 10;
...
function calculateCellWithLargestTotalPower(size: number, sn: number): string {
  ...
  for (let x = 1; x <= dimension; x++) {
    for (let y = 1; y <= dimension; y++) {
      const totalPower = calculateTotalPower(x, y, sn);
...
printSolution(__filename, `The coordinate of the top-left cell of the 3x3 grid with the largest total power: ${solution}`);